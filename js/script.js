...
function noodle(pathX, pathY, locationValue, length, iteration, valueSecondary)
{
  if (!Number.isInteger(pathX) || !Number.isInteger(pathY) ||
    !Number.isInteger(length) || !Number.isInteger(iteration))
  { throw new TypeError("Invalid arguments"); }
  let moveDir = (x, y, directionInt) =>
  {
    if (directionInt == 1) { y++; }
    if (directionInt == 2) { x--; }
    if (directionInt == 3) { x++; }
  ...
  let pathBias = (pathX, pathY) =>
  {
    if (pathX == -length && pathY == 0)
    { nodePathWeights = [1, 1, 0, 1]; }
    if (pathY == length && pathX == 0)
    { nodePathWeights = [1, 1, 1, 0]; }
    if (pathX == length && pathY == 0)
    { nodePathWeights = [1, 0, 1, 1]; }
    if (pathY == -length && pathX == 0)
    { nodePathWeights = [0, 1, 1, 1]; }
    return nodePathWeights;
  };

  let nodePathWeights = [1, 1, 1, 1];
  let nodeSecondWeights = [1, 1, 1, 1];
  const getDirection = prevDir =>
  {
    let lstDir = [1, 2, 3, 4];
    nodePathWeights[5 - prevDir - 1] = 0;
    nodePathWeights[prevDir - 1] = 1.25;
    return lstDir;
  };
  let prevPathDir = randomRandChoices([1, 2, 3, 4], nodePathWeights);
  iteration = iteration || 32;
  let secondaryX, secondaryY;
  for (let n = 0; n < iteration; n++)
  {
    prevPathDir = n ? pathDir : prevPathDir;
    let prevPathDirWeight = getDirection(prevPathDir);
    if (Math.abs(pathX) - 5 === 0 || Math.abs(pathY) - 5 === 0)
    { nodePathWeights = pathBias(pathX, pathY); }
    else { nodePathWeights = nodePathWeights; }
    var pathDir = randomRandChoices(prevPathDirWeight, nodePathWeights);
    for (let m = 0; m < length; m++)
    {
      [pathX, pathY] = moveDir(pathX, pathY, pathDir);
      player.biomeSet(pathX, pathY, locationValue);
    }
    nodePathWeights = nodePathWeights.map(node => node === 0 ? 1 : node);
    if (valueSecondary && n)
    {
      nodeSecondWeights = nodePathWeights;
      if (prevPathDir === pathDir)
      {
        nodeSecondWeights[pathDir - 1] = 0;
        nodeSecondWeights[5 - pathDir - 1] = 0;
      }
      [secondaryX, secondaryY] = moveDir(pathX, pathY,
        randomRandChoices([1, 2, 3, 4], nodeSecondWeights));
      if (Math.abs(secondaryX) >= length / 2 && Math.abs(secondaryY) >= length / 2 &&
        player.locationType[`${secondaryX}, ${secondaryY}`] !== locationValue)
      { player.biomeSet(secondaryX, secondaryY, valueSecondary); }
    }
  }
};
...
(function generateWorld()
{
  ...
  // @ Road, Enemy Hut
  let pathX = 0; let pathY = 0;
  const PathLength = 5, iteration = 127;
  noodle(pathX, pathY, enumBiome["Path"],
    PathLength, iteration, enumBiome["Enemy Hut"]);
